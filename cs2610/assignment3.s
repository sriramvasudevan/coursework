...
;this sets up ds, the stack segment and esp
mov ax,0x18
mov ds,ax
mov ax,0x38
mov ss,ax
...
;This throws exception 0
mov ax,0x12
mov bl,0x0
div bl
mov dword [ds:4],0xaaaa

;This throws exception 11
mov ax,0x20
mov es,ax ;present bit of es is set to 0

;This throws exception 13
mov dword[es:0x22ff], 0x0


jmp chanHappy

;This handles exception 0
align 0x100
mov dword [ds:0],0xbbbb
mov bl,0x3
iretd

;This handles exception 11
align 0x100
mov dword[ds:0x2024], 0x409200	;401200 -> 409200. This sets present bit to 1.
mov dword [ds:8], 0xcccc ; for testing
pop ebp ;pop from the stack
iretd

;This handles exception 13
align 0x100
mov dword[ds:0x2020], 0xb3203000 ;This increases the limit of es segment
mov ax, 0x20
mov es, ax ;This is to handle the shadow register
mov dword [ds:12], 0xdddd
pop ebp
iretd
...
;Data Segment Descriptor of type 0, random DPL and Base
 dd 0xb0002080 , 0x409200 
; Offset 0x20
;Data Segment Descriptor of type 1, random DPL and Base
;The present bit of this segment is set to 0
;We will make ES point to this
;Base Address :  0x0000b320
;Limit : 2400 = 0x0095f
;G, D/B, AVL, P = 0100
;DPL = 00 (PL0)
;S = 1
...
;Base Address (15-0)  : 0xb320
;Limit (15-0)         : 0x095f
;Base Address (31-24) : 0x00
;G-D/B-0-AVL          : 0x4
;Limit (19-16)        : 0x0
;P-DPL-S              : 0x01 PRESENT BIT IS 0
;Type                 : 0x2
;Base Address (23-16) : 0x00

 dd 0xb320095f , 0x401200
; Offset 0x28
...
;Descriptor for code segment, DPL = 0 
;Index 0x40
;Entries that we must fill are:
;Base Address:  0x0000a100
;Limit: 0x00200 ( 512 )
;G = 0, D/B = 1, AVL = 0, P = 1
;DPL = 00 (PL0)
;S = 1
;Type = 1010 (Read Execute/Code)

;Base Address ( bits 15-0)  : 0xa100
;Limit ( bits 15-0)         : 0x0300
;Base Address ( bits 31-24) : 0x00
;G-D/B-0-AVL                : 0x4
;Limit ( bits 19-16)        : 0x0
;P-DPL-S                    : 0x9
;Type                 	    : 0xa
;Base Address ( bits 23-16) : 0x00
...
;Interrupt Descriptor for trap 0 (division by zero)

;Bytes from 1-8
;Offset (bits 7-0)		: 00000000
;Offset(bits 15-8)		: 00000000
;Segment selector ( bits 23-16) : 01000000
;Segment selector(31-24)	: 00000000
;Reserved			: 00000000
;P-DPL-0-F			: 10001111
;Offset ( bits 23-16)		: 00000000
;Offset ( bits 31-24)		: 00000000
 db 0x00
 db 0x00
 db 0x40
 db 0x00
...
;Interrupt Descriptor for trap 11 (segment not found)

;Bytes from 1-8
;Offset (bits 7-0)		: 00000000
;Offset(bits 15-8)		: 00000001
;Segment selector ( bits 23-16) : 01000000
;Segment selector(31-24)	: 00000000
;Reserved			: 00000000
;P-DPL-0-F			: 10001111
...
;Interrupt Descriptor for trap 13 (segment limit exceeded)

;Bytes from 1-8
;Offset (bits 7-0)		: 00000000
;Offset(bits 15-8)		: 00000010
;Segment selector ( bits 23-16) : 01000000
;Segment selector(31-24)	: 00000000
;Reserved			: 00000000
;P-DPL-0-F			: 10001111
