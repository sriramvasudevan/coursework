...
mov ax,0x28
mov es,ax ;This is the page directory
mov bx, 0
PageDir:
	mov dword[es:bx],0xf017
	add bx,4
	cmp bx,4096 ;There are 2^12 entries between 0xe000 and 0xefff
	jne PageDir
;Thus at this point, all the entries in the page directory point to the page table at 0xf000

mov ax, 0x18
mov ds, ax ;This is the page with very large address ( base 0x1a000)
...
; Rest of all odd pages map to page at 0xc000 
;The following loop maps executes this mapping.
mov bx, 0
mov ax,0x30
mov fs,ax ;This is the page table
OddEven:
	mov dword[fs:bx],0xb017
	add bx,4
...
mov dword [ds:0], 0xbaba ;This sets 0x1a000 to 0xbaba. This mapping happens from 0xf068 which goes to 0xb000
;We must show that the value at 0xb000 is now 0xbaba

mov ax,0x20
mov es,eax

;STEP-5: jumping to a large location, and showing that actually jumped only to  the next line in this code
mov dword[fs:0x68], 0xa017 ; We have now mapped 0x1a000 to 0xa000

jmp 0x40: dword large_add - 0xa000 ;This will jump to the offset large_add - 0xa000 in the segment 0x18, which has base address 0x1a000
large_add:
	mov dword [es:0], 0xaaaa ;test move
	jmp 0x8: dword backtoCS - 0xa000 ;goes back to Code segment
...
mov dword [fs:0x68],0xb016 ; 
INVLPG [ds:4]
mov dword [ds:4], 0xbaba
mov dword [es:4], 0xbbbb ;this will create an interrupt for page fault 
...
align 0x300
mov dword [fs:0x68], 0xb017 ; making the present bit of the 20th entry 1
INVLPG [ds:4]
mov ax,0x18
mov ds,ax ; updating the shadow register of gs
...
;Start of My GDT
gdt_start:
;Null and Code Descriptor with PL=0 ;0x0,0x8
...
; Default Data Segment Descriptor of type 2, DPL = 0;0x10
 ...
 db 0x40
 db 0x0
;Data Segment Descriptor of type 0, random DPL and Base ;Here, the base address is 0x1a000. This is the large address we access.
 dd 0xa0000fff , 0x409201 ;0x18
;Data Segment Descriptor of type 1, random DPL and Base ;page 2;
 dd 0xc0000fff , 0x409200 ;0x20
;Data Segment Descriptor of type 2, random DPL and Base ;page directory
 dd 0xe0000fff , 0x409200 ;0x28
;Data Segment Descriptor of type 3, random DPL and Base ;page table
 dd 0xf0000fff , 0x409200 ;0x30
;Data Segment Descriptor of type 4, random DPL and Base ;stack
 dd 0xcf70008f , 0x409600 ;0x38
...
; db 0x0
; db 0x0
dd 0xa0000fff , 0x409b01 ; 0x40, large address CS
...
db 0x00
db 0x03 ; offset = 0x0300 since the ISR is aligned to 0xa300
db 0x08
db 0x00 ; segment selector = 0x0008 points to 0xa000 
db 0x00
db 0x8f
db 0x00
db 0x00
